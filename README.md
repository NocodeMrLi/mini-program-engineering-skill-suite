<p align="center">
  <img src="assets/readme-cover.webp?v=3.1.10" alt="小程序开发工程技能套件 封面" width="100%">
</p>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-3.1.10-0EA5E9.svg" alt="Version: 3.1.10">
</p>
...
如果你想先快速了解这套 Skill 解决什么问题、从哪里沉淀出来、适合怎么用，可以先看这个 [32 秒说明视频](https://raw.githubusercontent.com/NocodeMrLi/mini-program-engineering-skill-suite/main/assets/readme-promo.mp4)。
...
- **平台规则保鲜**：执行发布、提审、隐私申报等平台触点步骤时，按官方现行规则执行，不被套件内置的旧规则带偏（见下文「平台规则保鲜」）。
...
- **执行时永远以官方为准**：涉及上传、提审、审核、发布、隐私申报、配额等平台触点步骤时，Agent 会先核对套件记录的平台事实是否仍然新鲜（每条事实带核验日期与来源指纹）；过期或高风险步骤直接查官方现行文档再执行。因此**你本地安装的版本稍旧，也不会导致任务按过期规则执行**——版本旧只影响查官方的频率，不影响正确性。
...
- **平台规则保鲜**：执行以官方现行规则为准，套件内置规则只作缓存不作权威（2.0 起）。
...
```bash
tar -xzf mini-program-engineering-suite-v3.1.10.tar.gz
python3 mini-program-engineering-suite-v3.1.10/scripts/verify_public_package.py \
  mini-program-engineering-suite-v3.1.10
```
