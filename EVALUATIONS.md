...
| 版本 | 日期 | 摘要 | 说明 |
| --- | --- | --- | --- |
| 3.1.8 | 2026-08-31 | 全部门禁 PASS（含治理门禁真实失败路径验证，见下） | codex 七次复核批（1 P0+2 P1+2 P2+1 优化：P0=正式 Release 路径核验门被 checkout-tag 场景绕过→真 baseline=候选^+HEAD 校验+HOLD 阻断+未知值 fail-closed/四元组三者全等+拒重复 rule_id/重复规则 ID+重复核对点拒绝+七格矩阵/summary 记第四道门/文档口径以工件为准更正）；九个子 Skill 文本与评测基架零变更，六项复用 3.1.7；173 单测全绿；**P0 修复以真实失败 Release 验证**（临时 tag 删证据→工作流必须红）；独立终审与签署 PASS |
...
| 3.1.6 | 2026-08-31 | 全部门禁 PASS（精准最小，见下） | codex 五次复核批（3 P1 + 4 P2 + 1 P3：gate2 六维契约绑定/核验证据绑 CHANGELOG tag+分类集合化/提取器栈扫描弹出/release tag 整体匹配/抖音 28 条与 URL 二次修正/文档反向指导清理）；九个子 Skill 文本与评测基架零变更，六项复用 3.1.5；161 单测全绿（+5）；douyin 二次核验记录于本版条目；独立终审与签署 PASS |
| 3.1.5 | 2026-08-30 | 全部门禁 PASS（精准最小，见下） | codex 三次复核批（4 P1 + 6 P2 残留全修 + Gate5 方案 A 重构 + manual-only 首次人工核验）；变更均在工具链/workflow/平台数据，九个子 Skill 文本与评测基架零变更，六项复用 3.1.4；156 单测全绿（+9）；微信基线重录（旧空壳指纹根因修复）；alipay/douyin 首次核验并修正 3 个失效 URL；独立终审与签署 PASS |
| 3.1.4 | 2026-08-30 | 全部门禁 PASS（精准最小，见下） | 上线前质检第二批（5 项延期加固：Actions SHA 固定/verify 版本绑定/安装器事务性/扫描大小上限/i18n 事实对齐）；变更均在工具链与 workflow，九个子 Skill 文本与评测基架零变更，六项复用 3.1.3；143 单测全绿（+3）；全门禁与安装器正负路径实测；独立终审与签署 PASS |
| 3.1.3 | 2026-08-30 | 全部门禁 PASS（精准最小，见下） | 上线前质检交叉验证批（10 项实修 + 1 项证伪免修）：修复均在漂移工具链（scripts/ 与 workflows/），九个子 Skill 文本与评测基架零变更，评测语义等价，六项复用 3.1.2；140 单测全绿（+5 回归）；drift-watch 实跑 actionable=0（修复前同日为 3，假阳性根因修复）；结构/扫描/导出复验全过；独立终审与签署 PASS |
