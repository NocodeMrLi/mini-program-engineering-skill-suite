...
2. 按 [平台证据层](references/platform-evidence-layers.md) 分别记录源码、构建产物、开发者工具、真机、云端、体验版、审核版和正式版证据。
...
6. 使用 [权限与隐私静态映射](assets/privacy-permission-matrix.md) 分开核对源码 API、客户端授权、配置声明、平台申报、用户拒绝与撤回路径；缺少高层证据时保持 `unknown`。
6. 无法访问官方资料或官方说明不明确时，标记为 `unknown`，给出待核验问题，不凭记忆给确定结论。
7. 按 [微信平台核对清单](assets/wechat-platform-checklist.md) 输出约束、证据、缺口、风险和下一步；只建议动作，不把建议写成已执行。
