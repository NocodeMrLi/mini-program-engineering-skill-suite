...
### Changed（Actions Node 24 运行时升级）

- 将 Release 与 drift-watch 三处 `actions/upload-artifact` 从固定的 v4.6.0/Node 20 SHA 升级为官方 v7.0.1/Node 24 SHA `043fb46d1a93c77aae656e7c1c64a875d1fc6a0a`，消除 GitHub 运行器的 Node 20 弃用与强制兼容告警。
- 新增工作流契约回归，禁止旧 v4.6.0 SHA 回流，并强制保留 v7.0.1 的不可变提交固定。
- v3.1.12 的全部放行证据继续有效；本版是发布日志尾检发现的非阻断依赖维护补丁，不改变套件行为、门禁阈值或发布物结构。

### Release boundary

- 本版本仅更新 GitHub 官方 Action 的固定依赖、回归测试和版本文档；九个子 Skill 行为正文与评测基架不变，继续复用 v3.1.2 八阶段 PASS 证据并单独签名。

## 3.1.12 - 2026-09-01

### Fixed（不可变发布回读权限边界）

- 修复发布后验收器误用仓库管理员级 `immutable-releases` 设置接口：GitHub Actions 内置 `GITHUB_TOKEN` 对该接口固定返回 HTTP 403，即使仓库已启用不可变发布。
- 工作流改为通过普通 Release API 读取当前已发布版本的 `immutable=true`；该字段既在 Actions token 权限内，又能证明发布后 tag 与资产已锁定。仓库级设置继续由仓库管理员在发布前独立回读，不把 owner-only 权限伪装成 Actions 可用能力。
- 新增契约回归，禁止发布工作流重新调用 owner-only 设置接口，并强制保留 tag Release 的 `.immutable` 检查。
- v3.1.11 已成功创建不可变 Release，四工件 SHA256 与 116 文件接收端复验通过，但最后的仓库设置接口调用因权限失败，导致 Release Actions 红灯；不可变对象不改写，本次正式全绿修复顺延 v3.1.12。

### Release boundary

- 本版本只修复发布后状态回读、契约测试和版本文档，不改变九个子 Skill 行为正文与评测基架；继续复用 v3.1.2 八阶段 PASS 证据，并为 v3.1.12 单独签名。
